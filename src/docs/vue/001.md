...
1. **模块扁平化（dedupe）**上一步获取到的是一棵完整的依赖树，其中可能包含大量重复模块。比如 A 模块依赖于 loadsh，B 模块同样依赖于 lodash。在 npm3 以前会严格按照依赖树的结构进行安装，因此会造成模块冗余。
   ...
   -- bar
   ---- lodash@version2
2. **安装模块**这一步将会更新工程中的 node_modules，并执行模块中的生命周期函数（按照 preinstall、install、postinstall 的顺序）。
3. **执行工程自身生命周期**当前 npm 工程如果定义了钩子此时会被执行（按照 install、postinstall、prepublish、prepare 的顺序）。
...
[参考答案](https://link.zhihu.com/?target=https%3A//gongchenghuigch.github.io/2019/09/14/awat/)
...
- 首先什么是HTTP协议?http协议是超文本传输协议，位于tcp/ip四层模型中的应用层；通过请求/响应的方式在客户端和服务器之间进行通信；但是缺少安全性，http协议信息传输是通过明文的方式传输，不做任何加密，相当于在网络上裸奔；容易被中间人恶意篡改，这种行为叫做中间人攻击；

- 加密通信：为了安全性，双方可以使用对称加密的方式key进行信息交流，但是这种方式对称加密秘钥也会被拦截，也不够安全，进而还是存在被中间人攻击风险；
...
- location.search -- 返回URL中的查询字符串部分。例如 [http://www.dreamdu.com/dreamd...](https://link.zhihu.com/?target=http%3A//www.dreamdu.com/dreamdu.php%3Fid%3D5%26name%3Ddreamdu) 返回包括(?)后面的内容?id=5&name=dreamdu
- location.hash -- 返回URL#后面的内容，如果没有#，返回空 location.host -- 返回URL中的域名部分，例如[http://www.dreamdu.com](https://link.zhihu.com/?target=http%3A//www.dreamdu.com)
- location.hostname -- 返回URL中的主域名部分，例如[http://dreamdu.com](https://link.zhihu.com/?target=http%3A//dreamdu.com)
- location.pathname -- 返回URL的域名后的部分。例如 [http://www.dreamdu.com/xhtml/](https://link.zhihu.com/?target=http%3A//www.dreamdu.com/xhtml/) 返回/xhtml/
- location.port -- 返回URL中的端口部分。例如 [http://www.dreamdu.com](https://link.zhihu.com/?target=http%3A//www.dreamdu.com/):8080/xhtml/ 返回8080
- location.protocol -- 返回URL中的协议部分。例如 [http://www.dreamdu.com](https://link.zhihu.com/?target=http%3A//www.dreamdu.com/):8080/xhtml/ 返回(//)前面的内容http:
...
https协议由 http + ssl 协议构成，具体的链接过程可参考[SSL或TLS握手的概述](https://link.zhihu.com/?target=https%3A//github.com/lvwxx/blog/issues/3)
...
2. `link`最大限度支持并行下载，`@import`过多嵌套导致串行下载，出现[FOUC](https://link.zhihu.com/?target=http%3A//www.bluerobot.com/web/css/fouc.asp/)
...
3. 设置容器元素伪元素进行清理[推荐的清理浮动方法](https://link.zhihu.com/?target=http%3A//nicolasgallagher.com/micro-clearfix-hack/)
...
![img](https://pic4.zhimg.com/80/v2-25384025e52553dd5d67bdb1df023c07_720w.jpg)
...
![img](https://pic3.zhimg.com/80/v2-d85b6212d761590f863300b37d59856e_720w.jpg)
